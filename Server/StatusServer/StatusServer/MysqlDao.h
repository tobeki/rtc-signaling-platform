...
public:
	MySqlPool(const std::string& url, const std::string& user, const std::string& pass,
		...
		schema_(schema), poolSize_(poolSize), b_stop_(false), fail_count_(0) {
		try {
			...
			check_thread_ = std::thread([this]() {
				while (!b_stop_) {
					checkConnectionPro();
					std::this_thread::sleep_for(std::chrono::seconds(60));		// 设置每隔一分钟检查一次
				}
				});
	...
	// 只在队列存取的时候加锁，并且等所有连接都检测完成后再统一处理失败重连
	void checkConnectionPro() {
		...
		while (processed < targetCount) {
			...
			bool healthy = true;
			//解锁后做检查/重连逻辑
			// 空闲超过 5 秒，就发一条最轻量的 SQL 心跳包（SELECT 1）去验证它是不是还活着
