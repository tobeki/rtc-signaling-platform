...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data)
{
	...
	std::cout << "user login uid is  " << fromuid << " applyname  is "
	...
	bool b_info = GetBaseInfo(base_key, fromuid, apply_info);

	//如果对方在本服务器，直接通知对方有申请消息
...
void LogicSystem::AuthFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data)
{
	...
	std::cout << "from " << fromuid << " auth friend to " << touid << std::endl;
	...
	//更新数据库添加好友
	MysqlMgr::GetInstance()->AddFriend(fromuid, touid, back_name);
...
void LogicSystem::DealMsg() {
	for (;;) {
		...
		//判断如果为关闭状态，则取出逻辑队列中所有数据，及时处理并退出循环
		if (_b_stop) {
...
{
	//从mysql获取好友申请列表
	return MysqlMgr::GetInstance()->GetApplyList(to_uid, list, 0, 10);	// 0和10只是测试用
}
