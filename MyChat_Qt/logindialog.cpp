...
void LoginDialog::initHead()
{
    ...
    painter.setRenderHint(QPainter::Antialiasing); // 设置抗锯齿，以及使圆角更平滑
...
void LoginDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj){
        ...
        if (error != ErrorCodes::SUCCESS){
            showTip(tr("登录，参数错误"),false);
            enableBtn(true);
            return;
        }
        auto name = jsonObj["user"].toString();
        ...
        _uid = si.Uid;
        _token = si.Token;
        qDebug()<< "user_name is " << name << " uid is " << si.Uid <<" host is "
