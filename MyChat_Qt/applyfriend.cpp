    ...
    ui(new Ui::ApplyFriend)
{
    ...
    //链接(lb_ed)输入标签回车事件
    connect(ui->lb_ed, &CustomizeEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);  // 回车
...
bool ApplyFriend::enventFilter(QObject *obj, QEvent *event)
{
...
{
    int index = text.indexOf(add_prefix);   //在 text 里找第一次出现该前缀的位置
    if (index != -1) {
...
void ApplyFriend::SlotApplySure()
{
    ...
    if (bakname.isEmpty()) {
        bakname = ui->name_ed->placeholderText();
    }
