    ...
    _state(ChatUIMode::ChatMode), _b_loading(false), _cur_chat_uid(0)
{
    ...
    ui->search_edit->addAction(searchAction, QLineEdit::LeadingPosition);   // (icon)放在头部
    ui->search_edit->setPlaceholderText("搜索");

    // 创建一个清除动作并设置图标
    QAction* clearAction = new QAction(ui->search_edit);
    clearAction->setIcon(QIcon(":/res/close_transparent.png"));     // 初始时是一个透明图片
    // 初始时不显示清除图标
...
// 让左侧聊天列表 chat_user_list 高亮指定 uid 的那一行，并同步记录到成员 _cur_chat_uid
void ChatDialog::SetSelectChatItem(int uid)
{
    ...
    if (uid == 0) {
        ui->chat_user_list->setCurrentRow(0);   // 高亮第一行
        QListWidgetItem* firstItem = ui->chat_user_list->item(0);   //把第 0 行的 QListWidgetItem 拿出来
        if (!firstItem) {
            return;
        }
...
//让右侧聊天页 chat_page 显示指定 uid 的联系人资料
void ChatDialog::SetSelectChatPage(int uid)
{
    ...
    //比 SetSelectChatItem 多了一层 先转公共基类 ListItemBase 的套路，目的是支持多种行类型（联系人、群聊、搜索标题、分割线等）。
    //判断转化为自定义的widget
...
{
    auto find_iter = _chat_items_added.find(msg->_to_uid);
    if (find_iter != _chat_items_added.end()) {
        ...
        //更新当前聊天页面记录(如果是当前聊天页面的好友发来的消息，就需要当场更新)
