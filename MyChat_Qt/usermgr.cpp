...
void UserMgr::AppendApplyList(QJsonArray array)
{
    ...
    for (const QJsonValue &value : array) {
        ...
        auto info = std::make_shared<ApplyInfo>(uid, name, desc,
                                                icon, nick, sex, status);
...
std::vector<std::shared_ptr<FriendInfo> > UserMgr::GetChatListPerPage()
{
    ...
    friend_list = std::vector<std::shared_ptr<FriendInfo>>(_friend_list.begin() + begin,
