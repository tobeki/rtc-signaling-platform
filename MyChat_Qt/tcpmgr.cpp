...
void TcpMgr::initHandlers(){
    ...
    _handlers.insert(ReqId::ID_ADD_FRIEND_REQ, [this](ReqId id, int len, QByteArray data){
        ...
        if (!jsonObj.contains("error")){
            int err = ErrorCodes::ERR_JSON;
            qDebug() << "ID_ADD_FRIEND_REQ Failed, err is Json Parse Err" << err ;
            emit sig_login_failed(err);
            return;
        }
        ...
        int err = jsonObj["error"].toInt();
        if (err != ErrorCodes::SUCCESS){
            qDebug() << "ID_ADD_FRIEND_REQ Failed, err is " << err;
            emit sig_login_failed(err);
            return;
        }

        // 可添加请求发送成功的提示界面 todo...
        qDebug() << "Add Friend REQ Success,发送好友请求成功";
    });
    ...
    _handlers.insert(ReqId::ID_NOTIFY_ADD_FRIEND_REQ, [this](ReqId id, int len, QByteArray data){
        ...
        auto apply_info = std::make_shared<AddFriendApply>(from_uid,
                                                           name, desc, icon, nick, sex);
    ...
    _handlers.insert(ID_NOTIFY_AUTH_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        auto auth_info = std::make_shared<AuthInfo>(from_uid, name, nick,
                                                    icon, sex);
...
void TcpMgr::slot_send_data(ReqId reqId, QByteArray dataBytes)
{
    ...
    QByteArray block;
    QDataStream out(&block, QIODevice::WriteOnly);	// out和block绑定
