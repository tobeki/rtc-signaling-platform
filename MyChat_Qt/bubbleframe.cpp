...
const int WIDTH_SANJIAO  = 8;  //小三角的宽
...
void BubbleFrame::setWidget(QWidget *w)
{
    if(m_pHLayout->count() > 0)
        ...
        m_pHLayout->addWidget(w);	// w是不包含三角的长方形
    }
}
...
void BubbleFrame::paintEvent(QPaintEvent *e)
{
    ...
    painter.setPen(Qt::NoPen);		// 取消画笔，画出来的是无边框的
    ...
    {
        QColor bk_color(158,234,106);	// 自己发设为绿色
        painter.setBrush(QBrush(bk_color));
        //画气泡
        ...
        //画三角
        QPointF points[3] = {
            QPointF(bk_rect.x()+bk_rect.width(), 12),	// 距离顶部12
