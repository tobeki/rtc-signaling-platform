    ...
    : QWidget(parent), m_role(role)
{
    ...
    if(m_role == ChatRole::Self) {
        ...
        pGLayout->addWidget(m_pNameLabel, 0,1, 1,1);	// 第0行第1列，占1行占1列
        pGLayout->addWidget(m_pIconLabel, 0, 2, 2,1, Qt::AlignTop);
        pGLayout->addItem(pSpacer, 1, 0, 1, 1);
        pGLayout->addWidget(m_pBubble, 1,1, 1,1);
        pGLayout->setColumnStretch(0, 2);	// 第0列比列为2
        pGLayout->setColumnStretch(1, 3);	// 第1列比例为3
    }else{
...
void ChatItemBase::setWidget(QWidget *w)
{
    ...
    pGLayout->replaceWidget(m_pBubble, w);      // 把Bubble换成w
    delete m_pBubble;
    m_pBubble = w;
}
